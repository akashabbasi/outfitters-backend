...
export class HelperDateService {
  ...
  diff(dateOne: Date, dateTwo: Date, options?: IHelperDateOptionsDiff): number {
    ...
    } else if (options && options.format === ENUM_HELPER_DATE_DIFF.SECONDS) {
      return diff.asSeconds();
    } else if (options && options.format === ENUM_HELPER_DATE_DIFF.HOURS) {
      return diff.asHours();
    } else if (options && options.format === ENUM_HELPER_DATE_DIFF.MINUTES) {
  ...
  format(date: Date, options?: IHelperDateOptionsFormat): string {
    return moment
      .tz(date, options && options.timezone ? options.timezone : this.timezone)
      .format(
